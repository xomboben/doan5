...
export class ProductComponent extends BaseComponent implements OnInit{
  ...
  ngOnInit(): void {
    this._api.get('/api/Product/get-all').takeUntil(this.unsubscribe).subscribe(data => {
      this.products = data;
    }); 
  }
  ...
  onSubmit(value) {
    ...
    if(this.isCreate) { 
        ...
        this._api.post('/api/Product/update-acount',tmp).takeUntil(this.unsubscribe).subscribe(res => {
  ...
  onDelete(row) { 
    this._api.post('/api/Product/delete-category',{IDCategory:row.iDCategory}).takeUntil(this.unsubscribe).subscribe(res => {
  ...
  public openUpdateModal(row) {
    ...
    setTimeout(() => {
      $('#createUserModal').modal('toggle');
      this._api.get('/api/Product/get-by-id/'+ row.acountId).takeUntil(this.unsubscribe).subscribe((res:any) => {
        this.Product = res; 
          this.formdata = this.fb.group({
            'IDProduct': ['', Validators.required],
      'NameProduct': ['', Validators.required],
